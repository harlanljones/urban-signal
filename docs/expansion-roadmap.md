...
| **Norfolk** `norfolk` | PERMITS `fahm-yuh4` (`issue_date`) · DEEDS `qva7-tzrf` FY27 sales (`transfer_date`) · 311 `nbyu-xjez` (`creation_date`, ADR 0004) · SLA `dpi6-sct5` (`business_opened_date`) | socrata ×4 | ~5 | 311 located by address string (ADR 0004); licenses carry native lat/lng, `where: location_address != 'NO NORFOLK ADDRESS REQUIRED 99999'` drops ~25% placeholder rows → ~96% geocoded; sales keyed addr+GPIN with null-H3 events like Cook County; rotate FY dataset ID each July |
...
| **Philadelphia** `philly` | PERMITS `permits` · 311 `public_cases_fc` · SLA `business_licenses` · DEEDS `rtt_summary` (`document_type='DEED'`) | carto ×4 | ~4 | CartoClient prereq; sentinel dates (yr 3200/9798) filtered in watermark queries; deeds scoped to `document_type='DEED'` (~1.1M price-bearing rows; US-130) |
| **Cincinnati** `cinci` | PERMITS `uhjb-xac9` (`issued`) · 311 `gcej-gmiw` (`created_on`) · SLA `ehdi-ajku` · DEEDS Hamilton County CSV (`SaleDate` synthesized) | socrata ×3 + csv | ~2 | address-geocoded permits (G5 threshold 95%); deeds address-only (ADR 0004), `Valid='Y'` filter, `SaleDate` split across 3 int cols — snapshot window re-pulled & deduped |
