...
- **Spine files I expect to need:** `apps/api/src/config.py`, `apps/api/src/spatial/city_registry.py`, `apps/api/src/spatial/cities/__init__.py`, `apps/api/src/producers/scheduler.py`, `apps/api/src/producers/dob_permits_producer.py`, `apps/api/src/producers/complaints_311_producer.py`, `apps/api/src/producers/sla_licenses_producer.py`, `apps/api/src/producers/deeds_acris_producer.py`, plus the excluded static dashboard `apps/dashboard/public/index.html`.
...
- 2026-08-23 — Graph project is indexed and has no parse gaps; `apps/dashboard/public/index.html` is excluded by design and will be read directly.
