...
- [x] G1: evidence claims resolve to concrete repository-backed sources
  CHECK: node apps/site/scripts/verify-agent-surface.mjs
...
- [x] G6: the marketing site builds and its JavaScript parses
  CHECK: bun run --cwd apps/site typecheck && node --check apps/site/src/observatory.js && bun run --cwd apps/site build
