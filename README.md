...
| :---: | :---: |
| ![All metros national view](docs/screenshots/dashboard-all-metros.png) | ![Philadelphia & New York City together](docs/screenshots/dashboard-multi-metro.png) |

| Metro focus via chip |
| :---: |
...
| Norfolk (5 Divisions) | DOWNTOWN_WATERFRONT, GHENT_WESTBURG, OCEAN_VIEW, CENTRAL_MILITARY_CIRCLE, SOUTH_NORFOLK_BERKLEY | Socrata | Socrata (geocoded fallback) | Socrata (business licenses; 96% geocoded) | Socrata (FY sales; rotate ID each July) |
...
| Austin (6 Divisions) | DOWNTOWN_CAPITOL, EAST_AUSTIN_MUELLER, SOUTH_AUSTIN_SOCO, NORTH_AUSTIN_DOMAIN, WEST_AUSTIN_HILLS, PFLUGERVILLE_ROUND_ROCK_EDGE | Socrata | Socrata | Socrata TABC (Travis County, ADR-0004 geocoded; US-136) | — county shell |
| Cincinnati (1 Division) | CINCINNATI_CORE | [Socrata](docs/research/socrata-sweep.md#cincinnati) | [Socrata](docs/research/socrata-sweep.md#cincinnati) | [Socrata](docs/research/socrata-sweep.md#cincinnati) | CSV (Hamilton County Auditor; daily) |
| Boston (4 Divisions) | BOSTON_CORE, CAMBRIDGE_SOMERVILLE, INNER_NORTH, INNER_SOUTH | [CKAN](docs/research/non-socrata-platforms.md#boston--ckan--permits-and-311-strong-no-sales) | [CKAN](docs/research/non-socrata-platforms.md#boston--ckan--permits-and-311-strong-no-sales) | CKAN Licensing Board (EPSG:2249 State Plane→WGS84, US-137) | — no verified sales feed |
| Baltimore (1 Division) | BALTIMORE_CORE | ArcGIS | ArcGIS (year-sliced) | ArcGIS (notifications-grade) | Socrata (MD SDAT; snapshot) |
| Montgomery County, MD (1 Division) | MONTGOMERY_CORE | [Socrata](docs/research/socrata-sweep.md#montgomery-county-md--data-montgomerycountymdgov--2-solid--1-weak) | — MC311 excluded: no coordinates | [Socrata](docs/research/socrata-sweep.md#montgomery-county-md--data-montgomerycountymdgov--2-solid--1-weak) | Socrata (MD SDAT; snapshot) |
...
| Kansas City (1 Division) | KANSAS_CITY_CORE | — no open endpoint | Socrata | Socrata (business licenses; snapshot) | — no verified sales feed |
...
| Minneapolis (6 Divisions) | MINNEAPOLIS_CORE, CALHOUN_ISLES, CAMDEN_UPPER_NORTH, NORTHEAST, SOUTHEAST_UNIVERSITY, SOUTH_SOUTHWEST | ArcGIS | ArcGIS (year-sliced) | ArcGIS (On/Off Sale liquor) | — no verified sales feed |
| Milwaukee (1 Division) | MILWAUKEE_CORE | CKAN CSV, ~2.3-mo publication lag (US-138) | — no open 311 feed | [ArcGIS](docs/research/nine-unidentified-metros-platform.md#milwaukee-wi--register-liquor-licenses--with-a-2-month-lag-caveat) (liquor licenses) | CKAN yearly CSV snapshot, address-geocoded (US-138) |
| Charlotte (1 Division) | CHARLOTTE_CORE | — Accela portal (no bulk API) | ArcGIS (native coords) | — no verified feed | — Mecklenburg Hub verified (no sales feed) |
| Pittsburgh (1 Division) | PITTSBURGH_CORE | [CKAN](docs/research/socrata-sweep.md#skipped-and-dead-ends) (WPRDC PLI) | [CKAN](docs/research/data-coverage-sweep-2026-08-25.md) (WPRDC 311 Data; native lat/lng, intraday) | — business-activity licenses (66.9% addressed; defer) | CKAN (WPRDC Allegheny sales; address-only) |
| San Diego (6 Divisions) | CENTRAL_SAN_DIEGO, COASTAL, CORONA_NORTH_COUNTY, EAST_COUNTY_SAN_DIEGO, MID_CITY, SOUTH_BAY | CSV (Development Services approvals) | CSV (Get It Done) | CSV (business tax certificates; snapshot) | — no open endpoint |
...
| Spokane / Spokane County (1 Division) | SPOKANE_CORE | Excel (ArcGIS-hosted XLS; address geocoding) | — no verified 311 feed | Socrata (WA LCB liquor renewals; native points) | ArcGIS (annual county sales layers; polygon centroid) |
| Dayton / Montgomery County (1 Division) | DAYTON_CORE | — no verified feed | ArcGIS (Hansen rolling 90-day service requests; native points) | — no verified feed | — no verified feed |
| Tulsa / Tulsa County (1 Division) | TULSA_CORE | — no verified feed | ArcGIS (Verint approximately 30-day rolling window; native points) | — no verified feed | — no verified feed |
...
| Boise / Ada County (1 Division) | BOISE_CORE | ArcGIS (residential-only; state-plane source requested as WGS84 with ADR 0004 fallback) | — no verified feed | — no verified feed | — no verified feed |
...
|   raw.municipal.permits | raw.municipal.311 | raw.municipal.sla | raw.municipal.deeds            |
|   raw.municipal.crime | raw.municipal.street_cut | raw.municipal.evictions | raw.municipal.str | dlq.* |
+-------------------------------------------------+-------------------------------------------------+
...
│   ├── product/                            # Multi-page product and architecture explorer (ADR 0006)
│   │   ├── pages/                          # Multi-page templates & page-level metadata
│   │   ├── public/                         # Static assets, facts.json, llms.txt, llms-full.txt, cities/ JSON data
│   │   ├── scripts/                        # Static site generator, city renderer, verification harnesses
│   │   └── src/                            # Styles, animations, client interactive behavior
...
├── scripts/
│   ├── backfill_loader.py                  # Production municipal feed backfill loader
│   ├── backfill_probe.py                   # Ingestion backfill validation probe
│   ├── export_dashboard.py                 # Exports dashboard HTML to Worker public asset
│   ├── export_site_facts.py                # Exports single-source-of-truth facts.json from registry
│   ├── feed_staleness_probe.py             # Registered feed freshness probe with future date guards
│   ├── interlock_gap.py                    # Interlock-gap metric over a git diff range
│   ├── rejection_recheck.py                # Automated candidate feed verification harness
│   ├── replay_lag_measure.py               # Replay lag time-based measurement script
│   ├── replay_load.py                      # Replay load testing harness
│   ├── rollover_drill.py                   # Year-slice rollover verification CLI
│   └── verify-site-content.mjs             # Content and agent surface validation
...
| `docs/adr/0002-multi-platform-expansion.md` | Ingestion platform expansion (ArcGIS, CKAN, CARTO) |
| `docs/adr/0003-turborepo-monorepo.md` | Monorepo structure, Bun workspaces, Turbo pipeline |
| `docs/adr/0004-address-geocoding.md` | Fallback address geocoding for un-geocoded municipal feeds |
...
| `docs/research/data-coverage-sweep-2026-08-25.md` | Data-coverage sweep: 12 register-ready feeds + 4 defers + skip confirmations (drives US-124…US-139) |
