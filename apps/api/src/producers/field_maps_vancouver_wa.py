"""Per-city field maps for Vancouver, WA (US-233), imported by the shared parsers.

Vancouver, WA is a ONE-FEED PARTIAL metro: PERMITS only, from the
``Permits_and_Code_Enforcement_Data_(public_view)`` FeatureServer/0
("Permit Data") on the city's AGOL org (``CityOfVancouverGISAdmin``,
``services.arcgis.com/oNvpY90qsPDizwkN``). GIS.cityofvancouver.us (311) is
token-gated and Clark County recorder deeds are a web app — both stay Tier 3.
...
* ``csm_issued_date`` is esriFieldTypeDate (epoch-ms → ISO) and is
  ...
  discipline).
* No site-zip column exists (``PRIM_ADDR`` is the address without zip) and
...
PII is dropped at the map: no owner/contractor columns exist on this layer.
"""
